...
let enemyTouchDamage = 8;
let enemyProjectileDamage = 6;
let enemySpeed = 52;

let bossContactDamage = 16;
let bossProjectileDamage = 14;
let bossSpeed = 36;
let bossShootCooldown = 1.6;
...
function applyDifficultySettings() {
    ...
    if (difficulty === "easy") {
        ...
        bossSpeed = 32;
        bossShootCooldown = 1.9;
    } else if (difficulty === "hard") {
        enemyTouchDamage = 12;
        enemyProjectileDamage = 9;
        enemySpeed = 60;

        bossContactDamage = 22;
        bossProjectileDamage = 19;
        bossSpeed = 44;
        bossShootCooldown = 1.25;
    } else {
        enemyTouchDamage = 8;
        ...
        bossProjectileDamage = 14;
        bossSpeed = 36;
        bossShootCooldown = 1.6;
    }
}
...
function setup() {
    ...
    startBGM("level3", 0.05);

    // 小怪环境音
    if (sounds.ghost) {
        sounds.ghost.loop = true;
        sounds.ghost.volume = 0;
        sounds.ghost.play().catch(() => {});
    }

    // Boss环境音
    if (sounds.boss) {
        sounds.boss.loop = true;
        sounds.boss.volume = 0;
        sounds.boss.play().catch(() => {});
    }
}
...
function act(dt) {
    ...
    if (!pause) {
        ...
        updateLevel3SpatialSounds();

        cam.focus(player.left, player.top);
    }
}

function updateLevel3SpatialSounds() {
    if (!player) return;
    ...
    if (sounds.ghost) {
        ...
        if (nearest) {
            updateSpatialSound(px, py, nearest.x, nearest.y, sounds.ghost, 280);
        } else {
            sounds.ghost.volume = 0;
        }
    }
    ...
    /// ===== Boss压迫音 =====
    if (sounds.boss) {
        if (bossSoundDisabled || bossSealAnimating || bossSealed || end || !boss || !boss.alive) {
            sounds.boss.pause();
            ...
            updateSpatialSound(px, py, bx, by, sounds.boss, 520);
        }
    }
}
...
function fadeOutBossSound() {
    if (!sounds.boss) return;

    let fade = setInterval(() => {
        sounds.boss.volume -= 0.05;

        if (sounds.boss.volume <= 0) {
            sounds.boss.volume = 0;
            sounds.boss.pause();
            clearInterval(fade);
        }
