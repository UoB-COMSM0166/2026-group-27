...
const SCENE_DIFFICULTY_SELECT = "difficulty_select";
...
const enemyCodexData = [
  {
    id: "littleGhost",
    name: "Mist Ghost",
    img: "assets/little ghost.png",
    desc: "A lost soul shaped by the mist. It attacks intruders in the maze."
  },
  {
    id: "boss",
    name: "Devourer in the Mist",
    img: "assets/boss.png",
    desc: "A soul-devouring curse master. It must be sealed with the Key and Lock."
  }
];
...
const difficultyData = [
  {
    id: "easy",
    name: "Easy",
    desc: "Fewer ghosts in Level 2. Level 3 enemies hit softer."
  },
  {
    id: "normal",
    name: "Normal",
    desc: "Balanced original difficulty."
  },
  {
    id: "hard",
    name: "Hard",
    desc: "More ghosts in Level 2. Level 3 enemies and boss hit harder."
  }
];
...
function preload() {
  ...
  levelPreviewImgs[1] = loadImage("assets/level1.png");
  levelPreviewImgs[2] = loadImage("assets/level2.png");
  levelPreviewImgs[3] = loadImage("assets/level3.png");
  ...
  for (let enemy of enemyCodexData) {
    codexImgs[enemy.id] = loadImage(enemy.img);
  }
}
...
function draw() {
  ...
  else if (currentScene === SCENE_DIFFICULTY_SELECT) drawDifficultySelect();
...
function loadPersistentState() {
  ...
  selectedDifficulty = localStorage.getItem(DIFFICULTY_KEY) || "normal";
...
function drawMainMenu() {
  ...
  addMenuButton(bx, by + 0 * (bh + rowGap), bw, bh, "Start Adventure", () => {
    changeScene(SCENE_DIFFICULTY_SELECT);
  });
...
function drawDifficultySelect() {
  ...
  textStyle(BOLD);
  textSize(px(38));
  text("DIFFICULTY SELECT", area.x + px(36), area.y + px(34));

  textStyle(NORMAL);
  textSize(px(19));
  fill(220, 200, 160);
  text("Choose how dangerous the mist should be.", area.x + px(38), area.y + px(96));
  ...
  for (let i = 0; i < difficultyData.length; i++) {
    const d = difficultyData[i];
    const x = innerX + i * (cardW + cardGap);
    drawDifficultyCard(d, x, innerY, cardW, cardH);
  }
...
function drawDifficultyCard(diff, x, y, w, h) {
  ...
  if (selected) {
    ...
    stroke(170, 100, 35);
    strokeWeight(4);
    rect(x + px(5), y + px(5), w - px(14), h - px(14), px(14));
  }
  ...
  textStyle(BOLD);
  textSize(px(30));
  text(diff.name, x + w / 2, y + px(38));

  textStyle(NORMAL);
  textSize(px(18));
  fill(90, 60, 35);
  textBox(diff.desc, x + px(24), y + px(105), w - px(48), px(90));

  addMenuButton(x + px(24), y + h - px(62), w - px(48), px(42), selected ? "Selected" : "Choose", () => {
    selectedDifficulty = diff.id;
    localStorage.setItem(DIFFICULTY_KEY, diff.id);
    showFlash(`${diff.name} difficulty selected`, true);
    playEquipTone();
...
function drawLevelCard(levelInfo, x, y, w, h) {
  ...
  textStyle(BOLD);
  textSize(px(22));
  textBox(levelInfo.title, x + px(18), y + px(18), w - px(36), px(34));

  textStyle(NORMAL);
  textSize(px(16));
  fill(90, 60, 35);
  textBox(levelInfo.subtitle, x + px(18), y + px(52), w - px(36), px(30));

  let previewX = x + px(18);
  let previewY = y + px(88);
  let previewW = w - px(36);
  let previewH = px(120);

  drawLevelPreview(levelInfo.level, previewX, previewY, previewW, previewH);

  fill(80, 45, 20);
  textSize(px(20));
  textStyle(BOLD);
  text("Main Goals:", x + px(18), y + px(226));
  ...
  for (let goal of levelInfo.goals) {
    textBox("• " + goal, x + px(18), yy, w - px(36), px(22));
    yy += px(21);
  }
...
function drawLevelPreview(levelNumber, x, y, w, h) {
  ...
  let img = levelPreviewImgs[levelNumber];
  ...
  stroke(230, 190, 95, 190);
  strokeWeight(px(1));
  rect(x + px(3), y + px(3), w - px(6), h - px(6));

  stroke(255, 230, 150, 80);
  rect(x + px(6), y + px(6), w - px(12), h - px(12));

  // 角落装饰
  stroke(235, 195, 100, 180);
  strokeWeight(px(1));

  let c = px(14);

  line(x + px(5), y + px(5), x + c, y + px(5));
  line(x + px(5), y + px(5), x + px(5), y + c);

  line(x + w - px(5), y + px(5), x + w - c, y + px(5));
  line(x + w - px(5), y + px(5), x + w - px(5), y + c);

  line(x + px(5), y + h - px(5), x + c, y + h - px(5));
  line(x + px(5), y + h - px(5), x + px(5), y + h - c);

  line(x + w - px(5), y + h - px(5), x + w - c, y + h - px(5));
  line(x + w - px(5), y + h - px(5), x + w - px(5), y + h - c);
...
function drawCharacterCard(charData, x, y, w, h) {
  ...
  textStyle(BOLD);
  textSize(px(28));
  text(charData.name, x + w / 2, y + px(205));

  textStyle(NORMAL);
  textSize(px(19));
  textBox(charData.desc, x + px(24), y + px(255), w - px(48), px(80));
...
// ==================== Codex ====================
function drawCodexSectionTitle(title, x, y) {
  push();
  ...
  stroke(190, 145, 70, 180);
  strokeWeight(1);
  line(x, y + px(34), x + px(180), y + px(34));
...
function drawCodexScene() {
  ...
  textSize(px(19));
  fill(220, 200, 160);
  text("Collected records are saved here.", area.x + px(38), area.y + px(96));

  const gridX = area.x + px(42);
  const gridY = area.y + px(145);
  const gridW = area.w - px(84);
  const gridH = area.h - px(225);

  const cardW = px(210);
  const cardH = px(230);
  const gap = px(24);

  const cols = max(1, floor((gridW + gap) / (cardW + gap)));

  const itemRows = ceil(codexData.length / cols);
  const enemyRows = ceil(enemyCodexData.length / cols);

  const sectionTitleH = px(44);
  const sectionGap = px(36);

  const itemSectionH = sectionTitleH + itemRows * cardH + (itemRows - 1) * gap;
  const enemySectionY = gridY + itemSectionH + sectionGap;

  const enemySectionH = sectionTitleH + enemyRows * cardH + (enemyRows - 1) * gap;
  const contentH = itemSectionH + sectionGap + enemySectionH;

  codexMaxScrollY = max(0, contentH - gridH);
  codexScrollY = constrain(codexScrollY, 0, codexMaxScrollY);

  push();

  drawingContext.save();
  drawingContext.beginPath();
  drawingContext.rect(gridX, gridY, gridW, gridH);
  drawingContext.clip();

  translate(0, -codexScrollY);

  drawCodexSectionTitle("Items", gridX, gridY);
  ...
  for (let i = 0; i < codexData.length; i++) {
    ...
    let x = gridX + col * (cardW + gap);
    let y = gridY + sectionTitleH + row * (cardH + gap);

    drawCodexCard(item, x, y, cardW, cardH, hasCodexItem(item.id));
  }

  drawCodexSectionTitle("Enemies", gridX, enemySectionY);

  for (let i = 0; i < enemyCodexData.length; i++) {
    let enemy = enemyCodexData[i];
    let col = i % cols;
    let row = floor(i / cols);

    let x = gridX + col * (cardW + gap);
    let y = enemySectionY + sectionTitleH + row * (cardH + gap);

    drawCodexCard(enemy, x, y, cardW, cardH, true);
  }

  drawingContext.restore();
  pop();

  if (codexMaxScrollY > 0) {
    let barX = area.x + area.w - px(34);
    let barY = gridY;
    let barH = gridH;

    noStroke();
    fill(80, 50, 25, 160);
    rect(barX, barY, px(6), barH, px(4));

    let thumbH = max(px(40), barH * (gridH / contentH));
    let thumbY = barY + (codexScrollY / codexMaxScrollY) * (barH - thumbH);

    fill(230, 180, 90, 220);
    rect(barX, thumbY, px(6), thumbH, px(4));
  }

  addMenuButton(area.x + px(36), area.y + area.h - px(58), px(220), px(46), "Back", () => {
    changeScene(SCENE_MAIN_MENU);
  });
}

function drawCodexCard(item, x, y, w, h, unlocked = true) {
  drawParchmentPanel(x, y, w, h);
...
function drawGoldenPanel(x, y, w, h) {
  ...
  strokeWeight(px(3));
  noFill();
  rect(x + px(4), y + px(4), w - px(8), h - px(8), px(14));

  stroke(235, 190, 90);
  strokeWeight(px(1));
  rect(x + px(14), y + px(14), w - px(20), h - px(20), px(8));

  noStroke();
  fill(255, 230, 150, 12);
  rect(x + px(14), y + px(14), w - px(24), px(22), px(8));
}
...
function drawParchmentPanel(x, y, w, h) {
  ...
  strokeWeight(px(2));
  noFill();
  rect(x + px(5), y + px(5), w - px(14), h - px(14), px(9));
...
function drawCharacterPortrait(charData, x, y, size, highlight) {
  ...
  noStroke();
  fill(0, 0, 0, 60);
  ellipse(x + size / 2, y + size + px(14), size * 0.55, px(16));

  if (highlight) {
    fill(255, 220, 120, 24 + sin(uiTime * 2) * 8);
    circle(x + size / 2, y + size / 2 + px(38), size * 1.4);
  }
  ...
  if (img) {
    ...
    // Lando / Eliza：正常人形比例
    if (charData.id === "lando") {
      drawW = size * 1.0;
      drawH = size * 1.45;
      offsetY = px(28);
    }

    if (charData.id === "eliza") {
      drawW = size * 1.0;
      drawH = size * 1.45;
      offsetY = px(28);
    }
    ...
    image(img, x + size / 2, y + size / 2 + offsetY, drawW, drawH);
...
function drawFlashMessage() {
  ...
  rect(width / 2 - px(190), px(102), px(380), px(42), px(14));
...
function mouseWheel(event) {
  if (currentScene === SCENE_CODEX) {
    codexScrollY += event.delta;
    codexScrollY = constrain(codexScrollY, 0, codexMaxScrollY);
    return false;
  }
}
...
function keyPressed() {
  ...
  if (currentScene === SCENE_MAIN_MENU) {
    if (key === '1') changeScene(SCENE_DIFFICULTY_SELECT);
  ...
  if (currentScene === SCENE_DIFFICULTY_SELECT) {
    if (key === '1') {
