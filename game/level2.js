...
function act(dt) {
    ...
    if (!pause) {
        ...
        updateEnemies(dt);
        updateLittleGhostSound();

        cam.focus(player.left, player.top);
    }
}

function updateLittleGhostSound() {
    if (!player || enemies.length === 0 || !sounds.ghost) {
        if (sounds.ghost) sounds.ghost.volume = 0;
        return;
    }
